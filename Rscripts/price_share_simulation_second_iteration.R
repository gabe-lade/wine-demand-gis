...
sink("logfile_keepall", append = FALSE, split=TRUE) 
source('Z:/NIELSENDATA/consumer_panel/codeSTATA/GianCarlo/Discrete Choice/Main/Rscripts/Demand_functions.R')

winedata <-read.csv('data_for_counterfactual_second_iteration.csv')
save(winedata, file = "data_for_counterfactual_second_iteration.RData")
load("data_for_counterfactual_second_iteration.RData")
cat('Data Loaded. ', '\n')
start_time <- Sys.time()
...
for (market_num in 1:2600) {
  ...
  write.csv(winedata_sim, "simulated_price_share_second_iteration.csv")
  end_time <- Sys.time()
